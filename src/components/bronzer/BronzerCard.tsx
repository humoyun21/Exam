...
import { blushDataSuccess } from "../../slice/categories";


const BronzerCard: React.FC = () => {
  const category = useSelector((state: RootState) => state.allDataReducer.cart || { article: [] }) // Ensure fallback if undefined
  const dispatch = useDispatch()

  const BronzerCategory = async () => {
    dispatch(getDataStart())
    try {
      const res = await categories.bronzerProducts()
      dispatch(blushDataSuccess(res))
